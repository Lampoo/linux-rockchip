...
#define VL6180_IOCTL_GETOFFSET		_IOR('p', 0x08, int8_t)
...
static void stmvl6180_read_calibration_file(void)
{
	...
	f = filp_open("/param/calibration/offset", O_RDONLY, 0);
	if (f && !IS_ERR(f) && f->f_dentry) {
		fs = get_fs();
		set_fs(get_ds());
	...
	is_sign = 0;
	f = filp_open("/param/calibration/xtalk", O_RDONLY, 0);
	if (f && !IS_ERR(f) && f->f_dentry) {
		fs = get_fs();
		set_fs(get_ds());
		/* init the buffer with 0 */
		...
		if ((xtalk_calib + 26) < 64 )
			VL6180x_RangeIgnoreConfigure(vl6180x_dev, 300, 64); //300mm and 0.5Mcps
		else
			VL6180x_RangeIgnoreConfigure(vl6180x_dev, 300, (xtalk_calib + 26)); //300mm and xtalk + 0.2 Mcps
		VL6180x_RangeIgnoreSetEnable(vl6180x_dev, 1);
...
static void stmvl6180_write_offset_calibration_file(void)
{
	...
	f = filp_open("/param/calibration/offset", O_WRONLY | O_CREAT, 0644);
	if (f) {
		fs = get_fs();
		set_fs(get_ds());
...
static void stmvl6180_write_xtalk_calibration_file(void)
{
	...
	f = filp_open("/param/calibration/xtalk", O_WRONLY | O_CREAT, 0644);
	if (f) {
		fs = get_fs();
		set_fs(get_ds());
...
static void stmvl6180_work_handler(struct work_struct *work)
{
	...
	if (data->enable_ps_sensor == 1) {
		...
		if (data->enableDebug) {
			...
			vl6180_errmsg("RangeStatus as 0x%x, range_start: 0x%x\n",
...
static irqreturn_t stmvl6180_interrupt_handler(int vec, void *info)
{
	...
	//vl6180_dbgmsg("stmvl6180_interrupt_handler\n");
				...
				unsigned int cmd, unsigned long arg, void __user *p)
{
	...
	case VL6180_IOCTL_GETOFFSET:
		vl6180_dbgmsg("VL6180_IOCTL_GETOFFSET\n");
		offsetint = VL6180x_GetOffsetCalibrationData(vl6180x_dev);
		if (copy_to_user((int8_t *)p, &offsetint, sizeof(int8_t))) {
			vl6180_errmsg("%d, fail\n", __LINE__);
			return -EFAULT;
		}
		vl6180_dbgmsg("GETOFFSET as %d\n", offsetint);
		break;
	/* disable */
...
static int stmvl6180_start(struct stmvl6180_data *data, uint8_t scaling, init_mode_e mode)
{
	...
	if (mode != NORMAL_MODE) {
		...
		VL6180x_RangeIgnoreSetEnable(vl6180x_dev, 0);
	}
...
static int stmvl6180_setup(struct stmvl6180_data *data)
{
	...
	data->input_dev_ps->name = "stmvl6180";
...
static void __exit stmvl6180_exit(void)
{
	...
	if (gp_vl6180_data) {
		...
		if (gp_vl6180_data->irq_enable){
			free_irq(gp_vl6180_data->irq, gp_vl6180_data);
		}
