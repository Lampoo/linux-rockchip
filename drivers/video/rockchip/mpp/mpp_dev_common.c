...
extern int rockchip_iommu_tlb_invalidate_global(struct device *owner);
static void rockchip_mpp_run(struct rockchip_mpp_dev *mpp)
{
	...
	if (mpp->iommu_enable)
		rockchip_iommu_tlb_invalidate_global(mpp->dev);
#endif
	if (mpp->ops->run)
		mpp->ops->run(mpp);
			       ...
			       u32 __user *req)
{
	...
	if (ret < 0) {
		mpp_srv_err_hdl_locked(mpp->srv);
				...
				unsigned long fault_addr, unsigned int status)
{
	...
	if (ctx) {
		...
		if (ctx->mpp) {
			struct rockchip_mpp_dev *mpp = reg->mpp;
			u32 len = mpp->hw_info->dec_reg_num;
