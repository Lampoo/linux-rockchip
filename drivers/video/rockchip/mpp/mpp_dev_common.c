...
static int vcodec_bufid_to_iova(struct rockchip_mpp_dev *mpp, const u8 *tbl,
				int size, u32 *reg, struct mpp_ctx *ctx)
{
	...
	if (tbl == NULL || size <= 0) {
		mpp_err("input arguments invalidate\n");
		return -1;
	}
	...
	for (i = 0; i < size; i++) {
		...
		if (mpp->mmu_dev)
			ret = ion_map_iommu(mpp->dev,
					    mpp->ion_client,
					    mem_region->hdl,
					    &mem_region->iova,
					    &mem_region->len);
		else
			ret = ion_phys(mpp->ion_client,
				       mem_region->hdl,
			      ...
			      int idx)
{
	...
	return vcodec_bufid_to_iova(mpp, tbl, size, reg, ctx);
}
...
void mpp_dump_reg(u32 *regs, int count)
{
	int i;
				...
				void __user *src, u32 size)
{
	...
	mpp_srv_pending_lock(mpp->srv, ctx);
...
static void rockchip_mpp_run(struct rockchip_mpp_dev *mpp)
{
	...
	mpp_time_record(mpp_srv_get_current_ctx(mpp->srv));
	if (mpp->ops->run)
		mpp->ops->run(mpp);
...
static void rockchip_mpp_try_run(struct rockchip_mpp_dev *mpp)
{
	...
	if (!mpp_srv_pending_is_empty(mpp->srv)) {
		...
		if (ret ==0 && !test_bit(HW_RUNNING, &mpp->state))
...
static irqreturn_t mpp_isr(int irq, void *dev_id)
{
	...
	clear_bit(HW_RUNNING, &mpp->state);
...
static int mpp_dev_remove(struct platform_device *pdev)
{
	...
	mpp_srv_lock(mpp->srv);
	cancel_delayed_work_sync(&mpp->srv->power_off_work);
	mpp_srv_power_off(mpp->srv);
