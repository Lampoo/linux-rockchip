						...
						void __user *src, u32 dwsize)
{
	...
	if (!mpp_srv_is_power_on(mpp->srv)) {
		mpp_debug(DEBUG_RESET, "add a dummy ctx\n");
		mpp_srv_pending_lock(mpp->srv, &enc->dummy_ctx->ictx);
	}
...
static int rockchip_mpp_rkvenc_prepare(struct rockchip_mpp_dev *mpp)
{
	...
	ictx = mpp_srv_get_pending_ctx(mpp->srv);
	ctx_curr = container_of(mpp_srv_get_current_ctx(mpp->srv),
				struct rkvenc_ctx, ictx);
	ctx_ready =
...
{
	struct rkvenc_ctx *ctx = container_of(mpp_srv_get_current_ctx(mpp->srv),
					      struct rkvenc_ctx, ictx);
	struct rockchip_rkvenc_dev *enc =
		container_of(mpp, struct rockchip_rkvenc_dev, dev);
	u32 reg;
...
static int rockchip_mpp_rkvenc_irq(struct rockchip_mpp_dev *mpp)
{
	...
	/*
	 * according to war running, if the dummy encoding running with timeout,
	 * we enable a safe clear process, and when safe clear interruption
	 * trigger, we reset the ip, and complete the war procedure.
	 */
	...
	if (irq_status) {
		...
		if (irq_status != 1) {
			...
			if (irq_status & RKVENC_TIMEOUT_ERROR) {
				/* time out error */
				mpp_write_relaxed(mpp, RKVENC_INT_MSK_TIMEOUT,
						  RKVENC_INT_MSK);
				mpp_debug(DEBUG_RESET,
					  "timeout occur, enable safe clear\n");
...
static int rockchip_mpp_rkvenc_probe(struct rockchip_mpp_dev *mpp)
{
	...
	enc->lkt_hdl = ion_alloc(mpp->ion_client,
				 LINK_TABLE_LEN * 4 * 256, MPP_ALIGN_SIZE,
	...
	ret = ion_phys(mpp->ion_client, enc->lkt_hdl,
		       dma_addr, &tmp);
	enc->lkt_cpu_addr =
		ion_map_kernel(mpp->ion_client, enc->lkt_hdl);

	enc->war_hdl = ion_alloc(mpp->ion_client,
				 MPP_ALIGN_SIZE,
				 MPP_ALIGN_SIZE,
				 ION_HEAP(ION_CMA_HEAP_ID),
				 0);
	...
	ret = ion_phys(mpp->ion_client, enc->war_hdl,
		       &enc->war_dma_addr, &tmp);
