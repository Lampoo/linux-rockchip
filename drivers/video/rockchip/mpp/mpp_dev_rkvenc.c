						...
						void __user *src, u32 dwsize)
{
	...
	if (!mpp_srv_is_power_on(mpp->srv) && enc->dummy_ctx != NULL) {
...
static int rockchip_mpp_rkvenc_done(struct rockchip_mpp_dev *mpp)
{
	...
	if (ctx == enc->dummy_ctx) {
		...
		atomic_sub(1, &mpp->srv->total_running);
...
static long rockchip_mpp_rkvenc_ioctl(struct file *filp, unsigned int cmd,
				      unsigned long arg)
{
	...
	case MPP_DEV_RKVENC_SET_COLOR_PALETTE:
		mpp_write(mpp, RKVENC_OSD_CLK_SEL_BIT, RKVENC_OSD_CFG);
		/* TODO, set palette here */
		mpp_write(mpp, ~RKVENC_OSD_CLK_SEL_BIT, RKVENC_OSD_CFG);
		break;
	default:
		break;
