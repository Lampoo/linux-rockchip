...
inline void mpp_srv_pending_locked(struct mpp_service *pservice,
				   struct mpp_ctx *ctx)
{
...
inline void mpp_srv_err_hdl_locked(struct mpp_service *pservice)
{
	mpp_srv_lock(pservice);
	atomic_sub(1, &pservice->total_running);
	mpp_srv_unlock(pservice);
}
EXPORT_SYMBOL(mpp_srv_err_hdl_locked);
...
inline struct mpp_session *mpp_srv_get_current_session(
